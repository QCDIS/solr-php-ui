<?php
   ...
   function get_datevalues(&$results, $params, $downzoom)
   {
       ...
       foreach ($results
           ->facet_counts
           ->facet_ranges
           ->file_modified_dt->counts as $facet => $count)
       {
           $newstart = $facet;
   ...
   <div id="ID_Facet_<?php echo $rndNumber ?>" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
      <div class="bg-white py-2 collapse-inner rounded">
         ...
         <?php
            ...
            if ($is_taxonomy == true)
            {
                ...
                foreach ($facetvalue_array as $facet_value)
                {
                    ...
                    for ($i = 0;$i < count($paths) - 1;$i++)
                    {
                        ...
                        echo '<a class="collapse-item" onclick="waiting_on();" href="' . buildurl($params, $selected_facet, array(
                            $fullpath
                        ) , 's', 1) . '">' . substrwords(htmlspecialchars($label) , 15) . '</a>';
                    }
                    $label = end($paths);
                    $taxonomy = explode("\t", $label);
                    $label = end($taxonomy);
            ...
            foreach ($results
                ->facet_counts
                ->facet_fields->$facet_field as $facet => $count)
            {
                ...
                if ($i < $facetlimit)
                {
                    ...
                    if ($pathfacet)
                    {
                        ...
                        $link_filter = buildurl($params, $pathfacet, array(
                            $link_value
                        ) , 's', 1);
...
<?php
   ...
   { // char is not newline, so reset newline counter and write char to result
       $first = false;
       $emptylines = 0;
   ...
   $cfg['facets']['location_wkt_ss'] = array(
   ...
   else if ($view == 'VisualizeWebsites' || $view == "VisualizeWebpages")
   {
   $limit = 100000;
   }
   ...
   $count_open_etl_tasks_extraction = (int)($results
       ->facet_counts
       ->facet_queries
       ->count_open_etl_tasks_extraction);
   $count_open_etl_tasks_ocr = (int)($results
       ->facet_counts
       ->facet_queries
       ->count_open_etl_tasks_ocr);
   }
   catch(Exception $e)
   {
        $error = $e->__toString();
   ...
   // mask query for Solr
   $solrquery = mask_query($standardDoc . $query, $cfg['facets']);
   ...
   */
   ...
   foreach ($selected_facets[$selected_facet] as $selected_value)
   {
       ...
       if (isset($facet_config['tree']) && $facet_config['tree'] == true)
       {
           ...
           if ($configured_facet != 'path')
           { // do not do that for not multivalued path index structure, where only part of path (without parents) per taxonomy field
   ...
   elseif (strpos($Lfacet, "ecosystem") || strpos($facet, "EMBRC") || strpos($facet, "AnaEE") || strpos($facet, "SIOS") || strpos($facet, "INTERACT") || strpos($facet, "ICOS") || strpos($facet, "CETAF") || strpos($facet, "ELIXIR	") || strpos($facet, "EMPHASIS	")
   !== false)
   {
       $result = "ecosystem";
   }
